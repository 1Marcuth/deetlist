...
class EventsPageScraper:
    ...
    def _parse_event(self, event_selector: Tag) -> dict:
        ...
        icon_url = f"{self._base_url}/{icon_src.removeprefix('../')}"
        page_url = f"{self._url}{event_selector.attrs['href']}"
    ...
    def scrape(self) -> dict:
        ...
        past_events_header_selector = soup.find("h3", class_ = "see_ev ev_past")
        ...
        past_event_selectors = past_events_header_selector.select("a.eata")
        ...
        past_events = [ self._parse_event(event_selector) for event_selector in past_event_selectors ]
